...
#gisaid_phased_filt %>% filter(submitting_lab == "UCLA")  %>% 
#  inner_join(qc_input2, by=c("strain"="read_group_name")) %>% 
#  select(strain,ct,date_fix.y,coverage_3x,total_read_count,coverage_mean_no_dedup,pangolin_lineage,probability.x,date_fix.x,pass) %>% 
#dplyr::rename(coverage=coverage_mean_no_dedup,probability=probability.x,date_obtained=date_fix.x,passed_qc=pass)

qc_input2 %>% select(sample_name_fasta,uid,ct,date_fix,total_read_count, coverage_mean_dedup,mapped_sars2_dedup,sars2_percent_dedup,mapped_sars2_dup,mapped_sars2_dup,sars2_percent_dup,duplication_rate,coverage_3x,coverage_5x,pass) %>%
  dplyr::rename(sample_name=sample_name_fasta,date_collected=date_fix, read_count=total_read_count, coverage_rate=coverage_mean_dedup,coverage_3x=coverage_3x,coverage_5x=coverage_5x,
                sars2_percent=sars2_percent_dedup)  %>%
  select(sample_name, ct, date_collected, read_count,sars2_percent, coverage_rate,coverage_3x, coverage_5x)# %>% 
 # write.table(file = "paper/tables//S1.tsv",col.names=T, row.names=F,quote = F,sep="\t")
...
qc_input2 %>% filter(pass) %>% ggplot(aes(x=date_fix)) + geom_histogram(binwidth = 7) + theme_bw() +  scale_x_date(date_labels  = "%b-%d",date_breaks = "4 weeks") + theme(text=element_text(size=16)) + ylab("Count") + 
  xlab("Date collected") + scale_fill_brewer(palette = "Set1",name="Library type")
ggsave(file="paper/figures/S3.png",dpi=300,width=16,height = 8)


#### Create table S2.

uploaded_sequences = readDNAStringSet(("data/all_sequences2.fasta"))
gisaid_df = data.frame(gisaid_id=names(uploaded_sequences), number=names(uploaded_sequences) %>% str_split(pattern = "/")   %>% purrr::map(function(x){x[3]}) %>% unlist() %>% 
                         str_split(pattern="CA-UCLA")  %>% purrr::map(function(x){x[2]}) %>% unlist() %>% as.numeric())
...
tmp = gisaid_phased_filt %>% filter(submitting_lab == "UCLA") 
strain_numbers_orig =  tmp$strain %>% str_split("LA-") %>% purrr::map(function(x){x[2]}) %>% unlist() %>% as.numeric()


gisaid_phased_filt %>% filter(submitting_lab == "UCLA") %>%   select(strain,pangolin_lineage,probability)  %>% mutate(number=strain_numbers_orig) %>% 
  inner_join(gisaid_df,by=c("number"="number")) %>%
  select(strain, pangolin_lineage,probability, gisaid_id) %>% 
