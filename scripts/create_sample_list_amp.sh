...
# Create samplei list. 
START_NUMBER=$1
ls *R1_t*fq.gz | awk -v pwd="$PWD" -v startnumber="$START_NUMBER" '{split($1,a,"\.fastq.gz");c=$1;b=gsub("R1","R2",c); printf "%s %s %s %03i\n",a[1],pwd"/"$1,pwd"/"c,NR + startnumber-1}'   > samples1.txt
folder=$(dirname $0)
...
Rscript $folder/create_next_metadata.R samples2.txt $OUTPUT $CLIN_IN 
#rm samples1.txt
#rm run_names.txt
#rm samples2.txt
